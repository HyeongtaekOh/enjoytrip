...
async function modifyUsername() {
  const { value: formValues } = await Swal.fire({
    title: "사용자 정보 변경",
    html: `
      <input id="swal-username" class="swal2-input" value="${user.value.username}" placeholder="사용자 이름">
      <input id="swal-email" class="swal2-input" value="${user.value.email}" placeholder="이메일">
  ...
  if (formValues) {
    ...
    updateMember(
      ...
      ({ data }) => {
        // 서버 업데이트 성공 후에 user.value를 업데이트합니다.
        user.value = data;
          ...
          <div class="row g-0">
            ...
            <div class="col-md-8">
              <div class="card-body text-start">
                ...
                <button class="btn" @click="modifyUsername"> 수정</button>
              </div>
            </div>
          </div>
        </div>
